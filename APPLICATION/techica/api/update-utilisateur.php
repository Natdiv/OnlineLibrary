...
if(isset($postdata) && !empty($postdata))
{
    ...
    if ($new_delai_en_jour === 0) {
        ...
        $sql_update = "UPDATE `utilisateurs` SET `username`='{$username}', $sql_pwd,`categorie`='{$categorie}' WHERE `id`='{$id}'";
            ...
            $sql_create_event = "CREATE EVENT $username ON SCHEDULE AT CURRENT_DATE + '00:00:00' + INTERVAL $new_delai_en_jour DAY DO UPDATE utilisateurs SET etat='inactive' WHERE username='{$username}'";
